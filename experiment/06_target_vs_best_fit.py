"""Preliminary experiment — target vs best-fit biomass per station (equifinality check).

For each station, overlay:
  - target  = synthetic pseudo-observation (the twin-experiment ground truth);
  - single  = biomass of the best individual from the *single-station* optimisation;
  - MERGED  = biomass of the best individual from the *joint multi-station* optimisation.

Goal: see whether, despite poor *parameter* recovery (e.g. γτr), the *biomass* solution is
nearly identical to the target — i.e. evidence of equifinality (many parameter sets, same fit).

Uses the pop=256 validation logbooks (NRMSE fix). Re-runs SeapoPym with the best parameters
on the 6-station grid (same pipeline as the pseudo-observation generation).

Output : experiment/figures/Figure_R_target_vs_bestfit.{pdf,png} + printed NRMSE table.
"""
...
T_END = "2019-12-31"
ZOOM = slice("2015-01-01", "2019-12-31")   # plot window (full period is fitted/scored)

# station id -> (latitude, display label), low to high latitude
...
# ---------------------------------------------------------------------------
# Forcing grid (same construction as the pseudo-observation generation)
# ---------------------------------------------------------------------------
...
# Precomputed (fixed, REF-based) initial conditions — the SAME the GA used (run_optimization.py).
ic = xr.open_zarr(DATA_DIR / "initial_conditions.zarr").load()


def _best_params(sid):
    df = pd.read_parquet(DATA_DIR / f"ga_logbook_{sid}_validation.parquet")
    idx = df[("Weighted_fitness", "Weighted_fitness")].idxmax()
    return df.loc[idx, "Parametre"].to_dict()


def run_model(params):
    fg = FunctionalGroupParameter(functional_group=[FunctionalGroupUnit(
        name="zooplankton",
        energy_transfert=params["energy_transfert"],
        functional_type=FunctionalTypeParameter(
            lambda_temperature_0=params["lambda_temperature_0"],
            gamma_lambda_temperature=params["gamma_lambda_temperature"],
    ...
    with NoTransportSpaceOptimizedLightModel.from_configuration(configuration=config) as model:
        model.run()
        model.state.compute()
        b = model.state.biomass.sel(T=slice(T_ANALYSIS_START, T_END), functional_group=0, drop=True).load()
    return b
...
# ---------------------------------------------------------------------------
target = xr.open_zarr(DATA_DIR / "pseudo_observations.zarr")["observed_biomass"]
print("Running MERGED best individual ...", flush=True)
merged_b = run_model(_best_params("MERGED"))

print("Running single-station best individuals ...", flush=True)
single_b = {sid: run_model(_best_params(sid)) for sid, _, _ in STATIONS}
...
for ax, (sid, lat, label) in zip(axes.flat, STATIONS):
    ...
    ax.plot(sz["T"].values, sz.values, color="#d62728", lw=1.2, ls="-", label=f"single best (NRMSE={n_s:.3f})")
...
fig.suptitle("Target vs best-fit biomass (single-station and MERGED) — zoom 2015–2019",
             fontweight="bold", fontsize=13)
plt.tight_layout(rect=[0, 0, 1, 0.97])
out_pdf = FIG_DIR / "Figure_R_target_vs_bestfit.pdf"
out_png = FIG_DIR / "Figure_R_target_vs_bestfit.png"
