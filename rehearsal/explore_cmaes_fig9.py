"""rehearsal/explore_cmaes_fig9.py — Figure 9 (equifinality) in OFFICIAL format, from CMA-ES bests.

Same layout/logic as fig09_equifinality.py, but the best individuals come from the seeded CMA-ES
cross-check (logbooks/cmaes/, best seed of 10). For each station, overlays target (pseudo-obs),
single-station best-fit biomass, and MERGED best-fit biomass — showing that very different
parameter sets (recruitment equifinality) give nearly the same biomass. EXPLORATORY: writes to
figures/exploratory/Figure_9_cmaes.png; never touches the official Figure_9.png.

Re-runs SeapoPym with the GA's exact setup (Light model + precomputed IC + implicit solver).
Run: .venv/bin/python rehearsal/explore_cmaes_fig9.py
"""
...
ROOT = HERE.parent
DATA = ROOT / "data"
LOGS = HERE / "logbooks" / "cmaes"            # CMA-ES best-seed logbooks
FIG = HERE / "figures" / "exploratory"
FIG.mkdir(parents=True, exist_ok=True)
WF = ("Weighted_fitness", "Weighted_fitness")
LAMBDA = int(4 + 3 * np.log(5))
...
print("Running MERGED best (CMA-ES) ...", flush=True)
merged_b = run_model(best_params("MERGED"))
print("Running single-station bests (CMA-ES) ...", flush=True)
...
fig.suptitle(f"Target vs best-fit biomass (single & MERGED) — CMA-ES best of 10 (λ={LAMBDA}) — zoom 2015-2019",
             fontweight="bold", fontsize=13)
plt.tight_layout(rect=[0, 0, 1, 0.97])
out = FIG / "Figure_9_cmaes.png"
